        ...
        @if ( $models )
            @foreach( $models as $model )
                <tr>
                    <td>{{ $counter-- }}</td>
                    <td>
                        <img style="width: 150px;height: auto" src="{{ $model->imageUrl('medium') }}">
                    </td>
                    <td>{{ $model->title }}</td>
                    <td>{{ JalaliHelper::jdate( 'H:i Y/n/d', strtotime( $model->created_at ) ) }}</td>
                    <td>
                        <a href='{{ route( 'admin.post.edit', [ 'post' => $model->id ] ) }}' class="btn btn-xs btn-primary data-table-row-tool mr-1" data-toggle="tooltip" data-placement="top" data-original-title="{{ lang('Edit') }}">
                            <i class="la la-pencil"></i>
                        </a>
                        <form action='{{ route( 'admin.post.destroy', [ 'post' => $model->id ] ) }}' method='post' class='are-you-sure data-table-tools-form'>
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
