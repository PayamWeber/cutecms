        ...
        @if ( $models )
            @foreach( $models as $model )
                <tr>
                    ...
                    <td>
                        <a href='{{ route( 'admin.page.edit', [ 'page' => $model->id ] ) }}' class="btn btn-xs btn-primary data-table-row-tool mr-1" data-toggle="tooltip" data-placement="top" data-original-title="{{ lang('Edit') }}">
                            <i class="la la-pencil"></i>
                        </a>
                        <form action='{{ route( 'admin.page.destroy', [ 'page' => $model->id ] ) }}' method='post' class='are-you-sure data-table-tools-form'>
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
