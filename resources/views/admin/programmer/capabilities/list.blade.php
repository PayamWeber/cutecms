{!! widget_before( [
    'title' => lang( 'Current Capabilities' ),
    'color' => 'success',
] ) !!}
<div class="row">
        ...
        @if ( $models )
            @foreach( $models as $model )
                <tr>
                    ...
                    <td>{{ $model->name }}</td>
                    <td>
                        <a href='{{ route( 'programming.capabilities.edit', [ $model->id ] ) }}' class="btn btn-xs btn-primary data-table-row-tool mr-1" data-toggle="tooltip" data-placement="top" data-original-title="{{ lang('Edit') }}">
                            <i class="la la-pencil"></i>
                        </a>
                        <form action='{{ route( 'programming.capabilities.destroy', [ $model->id ] ) }}' method='post' class='are-you-sure data-table-tools-form'>
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
